...
parser.add_argument('--learning_rate', type=float, default=.0002) # DCGAN paper original value
...
parser.add_argument('--load_models_save', type=bool, default=False, help='Should we load the saved models?')
...
if param.load_models_save and models_save_folder != '':
    G.load_state_dict(torch.load(models_save_folder))
    D.load_state_dict(torch.load(models_save_folder))
...
# Variables
x = Variable(torch.FloatTensor(param.batch_size, param.n_colors, param.image_size, param.image_size).cuda())
y = Variable(torch.FloatTensor(param.batch_size).cuda())
z = Variable(torch.FloatTensor(param.batch_size, param.z_size, 1, 1).cuda())
# This is to see during training, size and values won't change
z_test = Variable(torch.FloatTensor(param.batch_size, param.z_size, 1, 1).normal_(0, 1).cuda())
...
optimizerD = torch.optim.Adam(D.parameters(), lr=param.learning_rate, betas=(param.beta1, 0.999))
optimizerG = torch.optim.Adam(G.parameters(), lr=param.learning_rate, betas=(param.beta1, 0.999))
...
for epoch in range(param.n_epoch):
    for i, data_batch in enumerate(dataset, 0):
        ...
        if i % 20 == 0:
            vutils.save_image(images, '%s/run-%d/images/real_samples.png' % (param.output_folder, run), normalize=True)
            fake_test = G(z_test)
            vutils.save_image(fake_test.data, '%s/run-%d/images/fake_samples_epoch%02d_n%02d.png' % (param.output_folder, run, epoch, i / 50), normalize=True)
